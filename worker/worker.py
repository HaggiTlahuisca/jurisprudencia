...
ESPERA_PAUSA_SCJN = int(os.getenv("ESPERA_PAUSA_SCJN", str(20 * 60)))
...
def worker_loop():
    ...
    # Indices — sin sparse para evitar conflictos con indices existentes
    for nombre, fn in [
        ("registro en acervo_historico",
            lambda: acervo_historico.create_index("registro", unique=True)),
        ("(anio, tipo_tesis) en acervo_historico",
            lambda: acervo_historico.create_index([("anio", 1), ("tipo_tesis", 1)])),
        ("vectorizado en acervo_historico",
            lambda: acervo_historico.create_index("vectorizado")),
        ("doc_id en sources_tfja",
        ...
        try:
            ...
            print(f"Indice '{nombre}' ya existe o no se pudo crear: {e}")
