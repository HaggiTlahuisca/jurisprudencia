...
# Reintentos diferidos para errores 500
DIFERIDO_MINUTOS = int(os.getenv("DIFERIDO_MINUTOS", "60"))       # reintento en 60 min
NO_DISPONIBLE_DIAS = int(os.getenv("NO_DISPONIBLE_DIAS", "3"))    # abandona tras 3 dias
...
def tomar_siguiente(cola):
    """
    Toma el siguiente documento disponible:
    - estado "pendiente", O
    - estado "diferido" cuyo next_run_at ya llegó
    """
    ...
    return cola.find_one_and_update(
        ...
        {
            "$set": {"estado": "procesando", "tomado_en": ahora},
            "$inc": {"intentos": 1},
        },
...
def marcar_error(cola, filtro: dict, mensaje: str):
    """Error definitivo (no relacionado con disponibilidad de la SCJN)."""
    cola.update_one(
        filtro,
        {
            "$set": {
...
def marcar_diferido_o_no_disponible(cola, filtro: dict, mensaje: str):
    """
    Cuando la SCJN devuelve 500/502/503/504:
    - Si el documento lleva menos de NO_DISPONIBLE_DIAS dias desde creado_en
      → estado "diferido", next_run_at = ahora + DIFERIDO_MINUTOS
    - Si ya pasaron mas de NO_DISPONIBLE_DIAS dias
      → estado "no_disponible" (ya no se reintenta)
    """
    ahora = datetime.utcnow()
    doc = cola.find_one(filtro, {"creado_en": 1})
    creado_en = doc.get("creado_en", ahora) if doc else ahora
    dias_transcurridos = (ahora - creado_en).total_seconds() / 86400
...
def procesar_tesis(doc_cola):
    ...
    if resp is None:
        # Sin respuesta de red — diferir
        marcar_diferido_o_no_disponible(cola_tesis, {"registro": registro_id}, err or "Sin respuesta")
        return False, True

    if resp.status_code != 200:
        if resp.status_code in (404, 410):
            # No existe y nunca existirá — completar sin guardar
            marcar_completado(cola_tesis, {"registro": registro_id})
            return True, False

        if agotado and resp.status_code in RETRY_STATUS_CODES:
            # Error temporal de la SCJN — diferir o marcar no_disponible
            marcar_diferido_o_no_disponible(
                cola_tesis,
        ...
        # Otro error no recuperable — completar
        marcar_completado(cola_tesis, {"registro": registro_id})
        return True, False
    ...
    if not rubro or not texto:
        # Registro vacío — completar sin guardar
        marcar_completado(cola_tesis, {"registro": registro_id})
        return True, False
...
def worker_loop():
    ...
    try:
        ...
        pass
    # Indice para consultas de diferidos
    try:
        cola_tesis.create_index([("estado", 1), ("next_run_at", 1)])
    except Exception:
        pass
